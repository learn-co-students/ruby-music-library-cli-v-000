...
puts describe "Concerns::Findable" do
...
puts describe "Concerns::Findable" do
  ...
  puts describe ".find_by_name" do
    ...
    puts context "works exactly like a generic version of Song.find_by_name," do
  ...
  puts describe ".find_or_create_by_name" do
    ...
    puts context "works exactly like a generic version of Song.find_or_create_by_name:" do
      ...
      puts it "isn't hard-coded" do
        expect(Genre.find_or_create_by_name("shoegaze")).to be(genre)
      end

      puts it "invokes .find_by_name instead of re-coding the same functionality" do
        expect(Artist).to receive(:find_by_name)
        Artist.find_or_create_by_name("Slowdive")
      end

      puts it "invokes the extended class's .create method, passing in the provided name, if an existing match is not found" do
