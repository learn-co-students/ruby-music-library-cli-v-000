require 'pry'

# create class Song
class Song 
# create initialize method w/argument (name)
  def initialize(name)
  
  end 
# create attr_accessor for name

# create @@all = [] - class variable 

# create class methods for:
    # - self.all - will return contents of @@all array
    # - self.destroy_all - set @@all to an empty array

# create save instance method - this method will add Song instances to the @@all array

# create self.create class method - this will initialize a new song and save the song in the @@all array 

end 