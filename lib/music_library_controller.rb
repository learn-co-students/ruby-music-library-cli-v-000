...
def songs
     Song.all.each_with_index {|song, index| puts "#{index + 1}. #{song.details}"}
end
...
if artist_wanted != nil
  Artist.find_by_name(artist_wanted).songs.each {|song| puts song.details}
else
...
if genre_wanted != nil
  Genre.find_by_name(genre_wanted).songs.each {|song| puts song.details}
else
