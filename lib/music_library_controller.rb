...
class MusicLibraryController
  ...
  def list_songs
   song_array =  Song.all.sort do |song_a, song_b| #returns an array of songs sorted by song name
      song_a.name <=> song_b.name #sorts them alphabetized by name
    end
    Song.all.uniq.sort { |a, b| a.name <=> b.name }.each.with_index(1) do |song,index| #uniq before sort to solve multiple instances of the same song, dubug strategy, returns a copy of array without duplicates
      puts "#{index}. #{song.artist.name} - #{song.name} - #{song.genre.name}"  #iterating over the song.all that's been sorted
    end
  end
  ...
  def play_song
    ...
    if (1..Song.all.length).include?(input)
      ...
      puts "Playing #{song.name} by #{song.artist.name}" if song #modifier, checking to see if it's anything but nil
    end
  end
