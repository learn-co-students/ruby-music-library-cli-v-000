...
class MusicLibraryController
  ...
  def list_artists
    x = Artist.all.map {|artist| artist.name}.sort
    x.each.with_index(1) {|artist, i| puts "#{i}. #{artist}"}
  end

  def list_genres
    x = Genre.all.map {|genre| genre.name}.sort
    x.each.with_index(1) {|genre, i| puts "#{i}. #{genre}"}
  end
  ...
  def play_song
    ...
    x = Song.all.sort {|a,b| a.name <=>b.name}[input-1]
